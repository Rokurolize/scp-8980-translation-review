...
def _format_inline_source(value: str) -> str:
    if "`" in value:
        return value
    return f"`{value}`"
...
def render_markdown(segment: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append(f"**対象 EN:** {_format_inline_source(segment['source_en_path'])}")
    lines.append(f"**対象 JP:** {_format_inline_source(segment['source_jp_path'])}")
    lines.append("")
...
def render_issue(lines: list[str], issue: dict[str, Any]) -> None:
    ...
    if issue["evidence"]:
        ...
        for ev in issue["evidence"]:
            en_quote = ev.get("en_quote", "")
            jp_quote = ev.get("jp_quote", "")
            source_ref = ev.get("source_ref", {})

            if en_quote:
                en_values.append(en_quote)

            if source_ref.get("kind") == "line":
                line_values.append(_format_inline_source(str(source_ref["line"])))
            elif source_ref.get("kind") == "note":
                line_values.append(_format_inline_source(source_ref["note"]))

            if jp_quote and jp_quote != "なし":
                jp_values.append(jp_quote)
...
def validate_segment(
    ...
    for issue in segment["issues"]:
        ...
        if order in seen_orders.get(severity, set()):
            messages.append(ValidationMessage("warning", segment_id, f"重複した表示順: {order}"))
        seen_orders.setdefault(severity, set()).add(order)
...
def validate_issue(
    ...
    for ev_idx, evidence in enumerate(issue["evidence"]):
        ...
        if ref["kind"] == "line" and en_part:
            ...
            if normalize_text(en_part) not in normalize_text(source_line):
                messages.append(
                    ValidationMessage(
                        "warning",
                        segment_id,
        ...
        if ref["kind"] == "line" and jp_part:
            ...
            if normalized_jp not in normalize_text(source_line):
                messages.append(
                    ValidationMessage(
                        "warning",
                        segment_id,
